...
class AnonymousAuthProvider extends AuthProvider {
  ...
  Future<CloudBaseAuthState> signInAnonymously() async {
    ...
    String? uuid = await (cache!.getStore(cache!.anonymousUuidKey) as FutureOr);
    String? refreshToken =
        await (cache!.getStore(cache!.refreshTokenKey) as Future<dynamic>);
    CloudBaseAuthType? loginType =
        await (cache!.getStore(cache!.loginTypeKey) as Future<dynamic>);
