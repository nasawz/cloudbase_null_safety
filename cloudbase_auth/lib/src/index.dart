...
class CloudBaseAuth extends AuthProvider {
  ...
  Future<void> sendPhoneCode(String phoneNumber) async {
    if (_phoneAuthProvider == null) {
      _phoneAuthProvider = PhoneAuthProvider(super.core!);
    }

    // CloudBaseAuthState authState =
    await _phoneAuthProvider!.sendPhoneCode(phoneNumber);

    // return authState;
  }

  /// 手机号登录
  Future<void> signUpWithPhoneCode(
  ...
  Future<CloudBaseAuthState> signInWithPhoneCodeOrPassword(
    ...
    CloudBaseAuthState authState = await _phoneAuthProvider!
        .signInWithPhoneCodeOrPassword(phoneNumber, phoneCode, password);

    return authState;
  }

  Future<void> forceResetPwdByPhoneCode(
      {required String phoneNumber,
      required String phoneCode,
      required String password}) async {
    if (_phoneAuthProvider == null) {
    ...
    await _phoneAuthProvider!
        .forceResetPwdByPhoneCode(phoneNumber, phoneCode, password);
  }
  ...
  Future<CloudBaseAuthState?> getAuthState() async {
    ...
    int? refreshTokenExpire =
        await cache!.getStore(cache!.refreshTokenExpireKey);
