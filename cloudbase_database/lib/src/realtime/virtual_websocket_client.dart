...
typedef Future<Map> SendFunc(
    {Map? msg, bool? waitResponse, bool? skipOnMessage, num? timeout});
...
class VirtualWebSocketClient {
  ...
  Future<LoginResult?> _internalLogin({String? envId, bool? refresh}) async {
    ...
    LoginResult? loginResult =
        await this._login!(envId: envId, refresh: refresh);
    if (this._envId == null) {
  ...
  void _handleWatchEstablishmentError(
      {e, String? operationName, required Completer completer}) async {
    ...
    Future<Null> Function([bool]) retry = ([bool? refreshLogin]) async {
    ...
    try {
      ...
      if (this._availableRetries.checkLast! > 0) {
        this._availableRetries.checkLast =
            this._availableRetries.checkLast! - 1;
        this._scheduleSendACK();
      } else {
        this.closeWithError(CloudBaseException(
            code: ErrCode.SDK_DATABASE_REALTIME_LISTENER_CHECK_LAST_FAIL,
      ...
      if (sessionInfo!.currentEventId! >= change.id!) {
        ...
        if (i == len - 1 ||
          ...
          Snapshot snapshot = Snapshot(
              id: change.id,
              docChanges: docChanges as List<SingleDocChange>?,
              docs: docsSnapshot,
              msgType: msgType);
...
//        Console.warn('nextevent ${msg['msgData']['currEvent']} ignored, msg: $msg}');
      ...
      case 'CHECK_EVENT':
        {
          if (this._sessionInfo!.currentEventId! <
            ...
            this._waitExpectedTimer = Timer(
                Duration(
                    milliseconds: this._getWaitExpectedTimeoutLength!() as int),
                () {
              // must rebuild watch
              this._rebuildWatch();
            });
  ...
  void closeWithError(error) {
    ...
    String? queryID =
        this._sessionInfo != null && this._sessionInfo!.queryID != null
            ? this._sessionInfo!.queryID
            : '';
    this._onWatchClose!(this, queryID);
