...
class RealtimeWebSocketClient {
  ...
  // appConfig
  late num _realtimePingInterval;
  ...
  void _internalInitWebSocketConnection({required bool reconnect, num? availableRetries, required Completer completer}) async {
    try {
      ...
      if (availableRetries! > 0) {
        ...
        if (isConnected) {
          await Future.delayed(Duration(milliseconds: this._reconnectInterval as int));
          if (reconnect) {
  ...
  Future<void> _initWebSocketEvent() async {
    ...
    var onClose = (int code, String reason) {
      ...
      this._clearHeartbeat();
      switch (code) {
        case CloseEventCode.ReconnectWebSocket as int: {
          // just ignore
          break;
        }
        case CloseEventCode.NoRealtimeListeners as int: {
          // quit
          break;
        }
        case CloseEventCode.HeartbeatPingError as int:
        case CloseEventCode.HeartbeatPongTimeoutError as int:
        case CloseEventCode.NormalClosure as int:
        case CloseEventCode.AbnormalClosure as int: {
        ...
        case CloseEventCode.NoAuthentication as int: {
          this.closeAllClients(getWSCloseError(code, reason));
          break;
        }
        default: {
  ...
  _heartbeat([bool immediate = false]) {
    ...
    this._pingTimer = Timer(
        Duration(milliseconds: immediate ? 0 : this._realtimePingInterval as int), () async {
          try {
            ...
            this._pongTimer = Timer(
                Duration(milliseconds: this._realtimePongWaitTimeout as int), () {
  ...
  void _send({
    ...
    Null Function([Map<dynamic, dynamic>]) resolve = ([Map? value]) {
      hasResolved = true;
      if (timer != null) {
        timer.cancel();
      }
    ...
    if (timeout != null) {
      timer = Timer(Duration(milliseconds: timeout as int), () async {
  ...
  void close(num code) {
    ...
    if (this._ws != null) {
      this._ws!.close(code as int, CloseEventCodeInfo[code]!.name);
      this._ws = null;
    }
  }
